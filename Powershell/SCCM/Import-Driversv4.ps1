...
$SCCMDriverSourcePath = $BaseSourcePath + "\OSD\DriverSources"
$SCCMDriverPackageSourcePath = $BaseSourcePath + "\OSD\DriverPackages"
$OSArchitectures = Get-ChildItem -Directory -Path $SCCMDriverSourcePath
...
Function Create-RecursiveTxtFile
{
    ...
    {
    #$OSArchitecture = $OSArchitectures[0]  #For testing
    $Manufacturers = Get-ChildItem -Directory -Path $OSArchitecture.FullName
        ...
        # $Manufacturer = $Manufacturers[0]  # For testing
            ...
            foreach($model in $models)
            {
                ...
                $directories = Get-ChildItem -Directory -Path $model.FullName -Recurse
